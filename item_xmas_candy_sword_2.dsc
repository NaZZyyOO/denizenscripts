...
item_xmas_candy_sword_2_use:
    type: world
    debug: false
    events:
        on player right clicks block with:item_xmas_candy_sword_2:
          - if <player.item_in_hand.script.name.is[==].to[item_xmas_candy_sword_2]||null>:
            - if <player.has_flag[candyblade_cd]> = false:
              ...
              - wait 0.2s
              - execute as_server silent "my addmana <player.name> 3"
              - feed <player> amount:2
              - repeat 3:
                - wait 0.2s
                - playsound sound:entity_generic_eat volume:0.4 pitch:2 <player.location>
                - playeffect at:<player.location.add[0,1.65,0]> effect:item_crack special_data:item_xmas_candy_sword_2 quantity:3 offset:0.2 velocity:<player.location.direction.vector.div[10]>
